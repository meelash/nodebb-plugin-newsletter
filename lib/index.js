...
// Import base modules.
const { db, Emailer, User, privileges, Meta, SocketUser, winston, nconf, path, jwt, url, util } = require('./nodebb.js')

// Import privilege handling functions.
const { filterPrivilegesList, filterPrivilegesGroupsList, filterPrivilegesListHuman} = require('./privileges.js') 
...
exports.load = async data => {
  ...
  SocketUser.Newsletter = {}

  // The user clicked send on the Newsletter page.
  SocketUser.Newsletter.send = async (socket, data) => {
    const canSend = await privileges.global.can('plugin_newsletter:send', socket.uid)
    if (!canSend) {
      throw new Error('[[error:no-privileges]]')
    }

    let { subject, body, groups, override, topicUrl, blacklist, prefixTitle } = data
    let count = 0
    let sets
  ...
  SocketUser.Newsletter.getGroups = async (socket, data) => {
    const canSend = await privileges.global.can('plugin_newsletter:send', socket.uid)
    if (!canSend) {
      throw new Error('[[error:no-privileges]]')
    }
  ...
  SocketUser.Newsletter.getBlacklist = async (socket, data, next) => {
    const canSend = await privileges.global.can('plugin_newsletter:send', socket.uid)
    if (!canSend) {
      throw new Error('[[error:no-privileges]]')
    }

    return await db.get('plugin-newsletter:blacklist')
  }
}
...
exports.filterPrivilegesList = filterPrivilegesList
exports.filterPrivilegesGroupsList = filterPrivilegesGroupsList
exports.filterPrivilegesListHuman = filterPrivilegesListHuman
